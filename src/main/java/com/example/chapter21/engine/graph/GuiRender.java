...
import com.example.chapter21.engine.*;
...
//        ImFontAtlas fontAtlas = ImGui.getIO().getFonts();
        int width = 0; // Taken from fontAtlas
        int height = 0;
//        ByteBuffer buf = fontAtlas.getTexDataAsRGBA32(width, height);
        ByteBuffer buf = BufferUtils.createByteBuffer(0);
        buf.flip();
        texture = new Texture(width, height, buf);
...
//        ImGuiIO io = ImGui.getIO();
        scale.x = 2.0f / 640;// io.getDisplaySizeX();
        scale.y = -2.0f / 640; // io.getDisplaySizeY();
